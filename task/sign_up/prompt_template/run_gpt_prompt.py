...
def run_gpt_prompt_init_sign_up_v1(init_persona, output_save_dir):
    ...
    save_txt="init_sign_up_request_v2.txt")
...
def run_gpt_prompt_sign_up_v3_with_publicreputation(init_persona, output_save_dir):
    def create_prompt_input(init_persona):
        prompt_input = []
        prompt_input += [init_persona.scratch.learned]
        prompt_input += [init_persona.scratch.name]
        ...
        self_public_repu = publicreputationDB.get_target_public_reputation(init_persona.scratch.ID, role="resident")
        prompt_input += [self_public_repu]
    ...
    prompt_template = "prompt/with_publicreputation/sign_up_request_v2_with_publicreputation.txt"
    prompt_input = create_prompt_input(init_persona)
    prompt = generate_prompt_role_play(prompt_input, prompt_template)
    ...
    save_run_prompts_single(prompt_template, init_persona, gpt_param, prompt_input, prompt, output,
    save_dir=output_save_dir,
    save_txt="sign_up_request_v2_with_publicreputation.txt")
...
def run_gpt_prompt_decide_to_talk_v1_with_publicreputation(init_persona, target_persona, output_save_dir):
    def create_prompt_input(init_persona, target_persona):
        target_reputation = init_persona.reputationDB.get_targets_individual_reputation(
            target_persona.scratch.ID, "resident"
        )
        ...
        prompt_input += [target_persona.scratch.name]
        prompt_input += [json.dumps(target_reputation)]
        target_public_reputation = publicreputationDB.get_target_public_reputation(target_persona.scratch.ID, role="resident")
    ...
    def __func_clean_up(gpt_response, prompt=""):
        try:
            ...
            # 有时会返回None?
            response_str = "step 1: None; step 2: No"
            return response_str
    ...
    prompt_template = "prompt/with_publicreputation/decide_to_talk_v2_with_publicreputation.txt"
    prompt_input = create_prompt_input(init_persona, target_persona)
    prompt = generate_prompt_role_play(prompt_input, prompt_template)
    ...
    save_dir=output_save_dir,
    save_txt="decide_to_talk_v2_with_publicreputation.txt")
...
def run_gpt_prompt_create_chat_v1(init_persona, target_persona, output_save_dir):
    def create_prompt_input(init_persona, target_persona):
        prompt_input = []
...
def run_gpt_prompt_summarize_chat_v1(init_persona, target_persona, convo, output_save_dir):
    ...
    save_run_prompts_pair(prompt_template, init_persona, target_persona, gpt_param, prompt_input, prompt, output,
        save_dir=output_save_dir,
        save_txt="summarize_basic_chat_v2.txt")
...
def run_gpt_prompt_willingness_to_gossip_v1(init_persona, target_persona, convo, output_save_dir):
    def create_prompt_input(init_persona, target_persona, convo):
        prompt_input = []
        prompt_input += [init_persona.scratch.learned]
        prompt_input += [target_persona.scratch.name]
    ...
    prompt_template = "prompt/willingness_to_gossip_v2.txt"
    prompt_input = create_prompt_input(init_persona, target_persona, convo)
    prompt = generate_prompt_role_play(prompt_input, prompt_template)
    ...
    save_run_prompts_pair(prompt_template, init_persona, target_persona, gpt_param, prompt_input, prompt, output,
        save_dir=output_save_dir,
        save_txt="willingness_to_gossip_v2.txt")
