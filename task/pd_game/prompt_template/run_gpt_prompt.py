...
def run_gpt_prompt_stage3_player_evaluation_v1_with_gossip_with_publicreputation(
    init_persona,
    target_persona,
    init_persona_decision,
    target_persona_decision,
...
def run_gpt_prompt_stage4_player_gossip_willing_v1_with_publicreputation(
    init_persona,
    target_persona,
    self_decision,
    target_decision,
    ...
    def create_prompt_input(
        ...
        target_persona_reputation = init_persona.reputationDB.get_targets_individual_reputation(target_persona.scratch.ID, "player")
        prompt_input += [json.dumps(target_persona_reputation)]
        prompt_input += [publicreputationDB.get_target_public_reputation(target_persona.scratch.ID, role="Player")]
    ...
    prompt_template = "prompt/with_publicreputation/player_gossip_willingness_v1_with_publicreputation.txt"
    prompt_input = create_prompt_input(init_persona, target_persona, self_decision, target_decision, target_reflection)
    prompt = generate_prompt_role_play(prompt_input, prompt_template)
    ...
    # save_dir="./task/pd_game/output_with_publicreputation",
    save_dir=output_save_dir,
    save_txt="player_gossip_willingness_v1_with_publicreputation.txt")
...
def run_gpt_prompt_stage4_player_gossip_willing_v1_with_publicreputation_with_buffer(
    init_persona,
    target_persona,
    self_decision,
    target_decision,
    target_reflection,
    init_persona_learned,
    target_persona_reputation,
    target_persona_publicreputation,
    output_save_dir,
    verbose=True,
):
    def create_prompt_input(
        ...
        # # 需要拿到更新的数据
        # prompt_input += [publicreputationDB.get_target_public_reputation(target_persona.scratch.ID, role="Player")]
        prompt_input += [target_persona_publicreputation]
...
def run_gpt_prompt_stage2_game_result_v1_with_publicreputation(
    ...
    def create_prompt_input(player1: Persona, player2: Persona):
        ...
        prompt_input += [player1.name]
        prompt_input += [player1_repu]
        prompt_input += [publicreputationDB.get_target_public_reputation(player1.scratch.ID, role="Player")]
        prompt_input += [player2.name]
        prompt_input += [player2_repu]
