...
client = OpenAI(
    ...
    timeout=60.0 # 建议加个超时时间，防止网络抖动死等
)


# from utils import openai_api_key
# import httpx
# client = OpenAI(api_key=openai_api_key, http_client=httpx.Client(verify=False), timeout = 3600)
...
def save_run_prompts_pr_list(
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        f.write(
            ...
            f"init persona: {init_persona.name}\n"
            f"~~~ target_persona -----------------------------------------------\n"
            f"target persona: {target_persona.publicreputation_available_set}\n"
            f"~~~ gpt_param ----------------------------------------------------\n"
            f"{gpt_param}\n"
            f"~~~ prompt_input ----------------------------------------------\n"
            f"{prompt_input}\n"
