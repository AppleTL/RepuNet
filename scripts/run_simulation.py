...
def parse_args():
    ...
    parser.add_argument("--public-reputation", choices=["y", "n"], default="y", help="Use public reputation? (y/n)")
    parser.add_argument(
        "--sim-name",
...
def prepare_run_name(scenario: str, reputation: str, public_reputation: str, gossip: str, steps: int, sim_name: str | None) -> str:
    if sim_name:
        return sim_name
    ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]  # Millisecond precision for readability and uniqueness
    rep = reputation.upper()
    pub = public_reputation.upper()
    gos = gossip.upper()
    return f"{scenario}_rep{rep}_pub{pub}_gos{gos}_s{steps}_{ts}"
...
def run_simulation(sim_code: str, scenario: str, reputation: str, public_reputation: str, gossip: str, steps: int):
    server = Creation(
        sim_code,
        reputation,
        gossip,
        with_publicreputation=public_reputation,
        sim=SCENARIO_TO_TREATMENT[scenario],
    )
...
def main():
    ...
    if args.sim:
        ...
        run_name = prepare_run_name(scenario, args.reputation, args.public_reputation, args.gossip, steps, args.sim_name)
        sim_code = copy_seed_to_run(seed_step, run_name)

    print(
        f"[info] start: scenario={scenario} steps={steps} reputation={args.reputation} "
        f"public_reputation={args.public_reputation} gossip={args.gossip} run_id={run_name} sim_code={sim_code}"
    )
    run_simulation(sim_code, scenario, args.reputation, args.public_reputation, args.gossip, steps)
